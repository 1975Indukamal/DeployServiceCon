...
const DropdownCard = ({ stack, type, currentStack, pathname }: { stack: string, type: "interview" | "practice", currentStack: string, pathname: string }) => {
    ...
    return (
        <NavigationMenuLink asChild>
            <Link
                href={`/interview-hub/dashboard/${stack}/${type}`}
                className={cn(
...
const DashboardHeader = () => {
    ...
    const currentStack = pathParts[3] || ""; // Adjusted for /interview-hub/dashboard/[stack]

    return (
        <div className="w-full border-b border-dark bg-background/80 backdrop-blur-xl supports-[backdrop-filter]:bg-background/60">
            <div className="mx-auto flex h-14 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
                <Link href="/interview-hub" className="flex items-center gap-2 transition-transform hover:scale-[1.02] active:scale-[0.98]">
                    <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-primary text-white text-xs font-bold">
                        QA
                    </div>
                    <span className="font-bold text-lg tracking-tight bg-gradient-to-r from-foreground to-foreground/70 bg-clip-text text-primary">
                        Dashboard
                    </span>
                </Link>
                ...
                <div className="hidden lg:flex items-center gap-6">
                    <Link href="/" className="text-xs font-medium text-muted-foreground hover:text-primary transition-colors border-r border-border pr-6">
                        ← Back to ServiceCon
                    </Link>
                    <NavigationMenu>
                        <NavigationMenuList className="gap-2">
                            <NavigationMenuItem>
                                <NavigationMenuTrigger className="h-9 px-3 py-1 font-medium text-sm">Interview</NavigationMenuTrigger>
                                <NavigationMenuContent>
                                    <div className="w-[240px] p-3 bg-dark">
                                        <div className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground px-2">
                                            Select Technology
                                        </div>
                            ...
                            <NavigationMenuItem>
                                <NavigationMenuTrigger className="h-9 px-3 py-1 font-medium text-sm">Practice</NavigationMenuTrigger>
                                <NavigationMenuContent>
                                    <div className="w-[240px] p-3 bg-dark">
                                        <div className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground px-2">
                                            Select Technology
                            ...
                            {individualStacks.map((stack) => (
                                <NavigationMenuItem key={`nav-${stack}`}>
                                    <Link href={`/interview-hub/dashboard/${stack}/interview`} legacyBehavior passHref>
                                        <NavigationMenuLink
                                            className={cn(
                                                navigationMenuTriggerStyle(),
                                                "h-9 px-3 py-1 font-medium text-sm capitalize transition-all",
                ...
                {/* Mobile toggle */}
                <button
                    className="lg:hidden inline-flex h-9 w-9 items-center justify-center rounded-md text-foreground transition-colors hover:bg-accent hover:text-accent-foreground focus:outline-none"
            ...
            <div
                className={cn(
                    "lg:hidden absolute top-[100%] left-0 w-full overflow-hidden transition-all duration-300 ease-in-out border-b border-border bg-card shadow-lg z-50",
            ...
            >
                <nav className="flex flex-col gap-4 p-4 sm:p-6 overflow-y-auto max-h-[calc(100vh-4rem)]">
                    ...
                    <div className="flex flex-col gap-2">
                        <button
                            className="flex items-center justify-between px-4 py-3 text-sm font-semibold bg-primary/5 hover:bg-primary/10 rounded-xl transition-colors"
                            onClick={() => setMobileInterviewOpen(!mobileInterviewOpen)}
                        >
                            Interview Preparation
                            <ChevronDown className={cn("h-4 w-4 text-muted-foreground transition-transform duration-300", mobileInterviewOpen && "rotate-180")} />
                        </button>
                        <div
                            className={cn(
                        ...
                        >
                            {coreStacks.map((stack) => (
                                <Link
                                    ...
                                    className={cn(
                                        "flex flex-col justify-center px-4 py-3 rounded-xl text-xs font-medium capitalize transition-all border",
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        >
                            Practice Challenges
                            <ChevronDown className={cn("h-4 w-4 text-muted-foreground transition-transform duration-300", mobilePracticeOpen && "rotate-180")} />
                        </button>
                        <div
                            className={cn(
                                "grid grid-cols-1 gap-2 overflow-hidden transition-all duration-300 ease-in-out",
                        ...
                        >
                            {coreStacks.map((stack) => (
                                <Link
                                    ...
                                    href={`/interview-hub/dashboard/${stack}/practice`}
                                    onClick={() => setMobileOpen(false)}
                                    className={cn(
                                        "flex flex-col justify-center px-4 py-3 rounded-xl text-xs font-medium capitalize transition-all border",
                    ...
                    <div className="grid grid-cols-1 gap-2">
                        <h4 className="px-4 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground mb-1">
                            Specialized Topics
                        </h4>
                        {individualStacks.map((stack) => (
                            <Link
                                ...
                                onClick={() => setMobileOpen(false)}
                                className={cn(
                                    "flex items-center justify-between px-4 py-3 rounded-xl font-medium capitalize transition-all border text-sm",
                                    currentStack === stack
                                        ? "border-primary/50 bg-primary/10 text-primary shadow-sm"
                    ...
                    <Link
                        href="/"
                        onClick={() => setMobileOpen(false)}
                        className="flex items-center justify-center px-4 py-3 rounded-xl font-bold bg-muted text-foreground border border-border"
                    >
                        ← Back to ServiceCon
                    </Link>
                </nav>
